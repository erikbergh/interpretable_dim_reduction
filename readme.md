...
<br>
Let's reduce the data above to three dimensions.
<br>
...
array([[0.42104936],[0.20807356],[0.37087724]], dtype=float32)
<br><br>
...
plot_heatmap(df)
sns.scatterplot(num * (x[:,0] - min_x) / (max_x - min_x), num*(x[:,1] - min_y) / (max_y - min_y), c=S_points[:,1], cmap='Greens')
plt.show()
```
...
<br>
Here both the color of the green points show that the Y-axis is represented more at the middle bends of the S. The background color indicates that the Y-axis accounts for up to 20% of the distance between points at the middle bends.
<br><br>
...
plot_heatmap(df)
sns.scatterplot(num * (x[:,0] - min_x) / (max_x - min_x), num*(x[:,1] - min_y) / (max_y - min_y))
plt.show()
```
...
<br>
Here we see that the space is expanded at the middle of the S while contracted at the ends. This is important if one wants to compare distance between different parts of the reduced space.
<br><br>
...
<br>
Here we see that the transformation is less skewed at the middle bends, this since the original Y-axis is given more importance here. There seems to be a difference between the top and bottom of the S.
